...
export function planChunks(totalSize) {
...
export async function initVideoPost(accessToken, { title, privacyLevel, videoSize, chunkSize, totalChunkCount, disableComment, disableDuet, disableStitch }) {
...
export async function uploadVideoChunks(uploadUrl, videoBuffer, mimeType, chunkSize, totalChunkCount) {
  ...
  for (let i = 0; i < totalChunkCount; i++) {
    ...
    if (!res.ok) throw new Error(`Error al subir el fragmento ${i + 1}/${totalChunkCount} a TikTok`);
  }
}
